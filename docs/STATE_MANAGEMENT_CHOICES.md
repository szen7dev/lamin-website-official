...
# Giải Thích Lựa Chọn Công Nghệ State Management Cho Dự Án Lamin Pharmacy

## Mục Đích Tài Liệu

Tài liệu này giải thích chi tiết lý do lựa chọn các công nghệ state management cho dự án frontend Lamin Pharmacy. Mục tiêu là cung cấp thông tin rõ ràng và dễ hiểu cho các lập trình viên mới hoặc người đi sau dự án, giúp nắm bắt được quyết định lựa chọn công nghệ và rationale đằng sau.

## Tổng Quan State Management Trong Dự Án Lamin Pharmacy

Trong dự án Lamin Pharmacy, chúng ta phân loại state management thành 4 loại chính:
...
- **Đơn giản và Nhẹ Nhàng:** Phù hợp cho global state không quá phức tạp của MVP Lamin (authentication, theme, giỏ hàng).
...
- **Overkill cho MVP:** Các thư viện này quá mạnh mẽ và phức tạp cho global state đơn giản của MVP Lamin.
...
Lựa chọn công nghệ state management cho dự án Lamin Pharmacy được đưa ra dựa trên các tiêu chí:
