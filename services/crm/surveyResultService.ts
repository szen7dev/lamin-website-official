...
class SurveyResultService {
  ...
  async submitSurveyResult(
    ...
    try {
      ...
      const response = await apiClient.post<SurveyResult>(
        '/api/crm/survey_result',
        payload,
      );
  ...
  async getSurveyResultById(resultID: string): Promise<SurveyResult> {
    try {
      const response = await apiClient.get<SurveyResult>(
        `/api/crm/survey_result/${resultID}`,
      );
  ...
  async updateSurveyResult(
    resultID: string,
    data: Partial<CreateSurveyResultData>,
  ): Promise<SurveyResult> {
    try {
      const response = await apiClient.put<SurveyResult>(
        `/api/crm/survey_result/${resultID}`,
        data,
      );
  ...
  async getSurveyResults(
    params?: SurveyResultQueryParams,
  ): Promise<SurveyResult[]> {
    try {
      const queryParams = {
        optionSeller: DEFAULT_OPTION_SELLER,
