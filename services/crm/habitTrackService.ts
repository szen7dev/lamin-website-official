...
class HabitTrackService {
  ...
  async createHabitTrack(data: CreateHabitTrackData): Promise<HabitTrack> {
    try {
      ...
      const response = await apiClient.post<HabitTrack>(
        '/api/crm/habit_track',
        payload,
      );
  ...
  async updateHabitTrack(
    ...
    try {
      const response = await apiClient.put<HabitTrack>(
        `/api/crm/habit_track/${trackID}`,
        data,
      );
  ...
  async getHabitTracks(params?: HabitTrackQueryParams): Promise<HabitTrack[]> {
    try {
      ...
      const response = await apiClient.get<HabitTrack[]>(
        '/api/crm/habit_track',
        queryParams,
      );
