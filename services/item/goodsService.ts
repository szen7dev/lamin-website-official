...
class GoodsService {
  ...
  async updateGoods(
    goodsID: string,
    data: Partial<CreateGoodsData>,
  ): Promise<Goods> {
    try {
      const response = await apiClient.put<Goods>(
        `/api/item/goods/${goodsID}`,
        data,
      );
  ...
  async getRelatedProducts(
    categoryID: string,
    params?: Omit<GoodsQueryParams, 'categoryID'>,
  ): Promise<Goods[]> {
  ...
  async searchProducts(
    keyword: string,
    params?: Omit<GoodsQueryParams, 'keyword'>,
  ): Promise<Goods[]> {
