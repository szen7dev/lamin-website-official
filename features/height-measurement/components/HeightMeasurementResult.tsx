...
export default function HeightMeasurementResult({ resultId }: HeightMeasurementResultProps) {
  ...
  if (!data || !data.heightData) {
    return (
      <div aria-live="polite" className="flex items-center justify-center py-8 sm:py-12">
        <div className="flex flex-col items-center gap-3 sm:gap-4">
  ...
  return (
    <article
      ...
      <section className="flex-1 space-y-3 sm:space-y-4 order-1 md:order-2">
        {/* Row 1: Growth Rate */}
        <div aria-label="Đường tăng trưởng" className="flex items-center gap-2 sm:gap-4">
          ...
          <div
            ...
            {[...percentiles, { name: 'Dự đoán', color: '#198754' }].map(item => (
              <div
                ...
                <div className="h-0.5 w-3 sm:w-4" style={{ backgroundColor: item.color }} />
        ...
        <div className="space-y-1 sm:space-y-2 text-xs sm:text-sm">
          <p>
            • Chiều cao: {data.height}cm. Đạt chuẩn. Bé thấp hơn so với chiều cao trung bình là
            2,156cm. Chuẩn WHO: 114,156cm
          </p>
          <p>
            • Bé {data.gender === 'male' ? 'Nam' : 'Nữ'}, {data.name}, sinh ngày{' '}
            {new Date(data.birthDate).toLocaleDateString('vi-VN')} - {age.years} tuổi, {age.months}{' '}
            tháng, {age.days} ngày
          </p>
          ...
          <p>
            • Giải pháp tăng chiều cao (Khi con bạn trưởng thành. Chiều cao trung bình của bé trai
            là: 177cm và bé Bé Gái là: 163,5cm)
          </p>
          <p className="text-grayscale-60">
            • Con có thể không đạt được chiều cao dự đoán nếu bị ảnh hưởng bởi những thói quen sinh
            hoạt xấu
          </p>
          <p className="text-grayscale-60">
            • Con có thể tăng thêm 7 - 15cm so với dự đoán khi trưởng thành nếu bố mẹ giúp con áp
            dụng giải pháp tăng chiều cao của Lamin
          </p>
          <ul
            ...
            {data.recommendations.map((recommendation, index) => (
              <li key={index} className="flex items-center gap-1 sm:gap-2">
                <div className="flex h-4 w-4 sm:h-5 sm:w-5 items-center justify-center rounded-full bg-success-5">
                  <Check aria-hidden="true" className="h-2 w-2 sm:h-3 sm:w-3 text-white" />
                </div>
