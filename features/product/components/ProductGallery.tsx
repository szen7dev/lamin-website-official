...
export default function ProductGallery({ images }: ProductGalleryProps) {
  ...
  const processedImages = useMemo(() => {
    return images.map((image, index) => ({
      ...
      url: image.url || `/placeholder.svg?height=400&width=400&unique=${index}-${Date.now()}`,
  ...
  const remainingImagesCount = Math.max(0, processedImages.length - MAX_THUMBNAILS)

  const handlePrevious = () => {
    setCurrentImage((prev) => (prev === 0 ? processedImages.length - 1 : prev - 1))
  }

  const handleNext = () => {
    setCurrentImage((prev) => (prev === processedImages.length - 1 ? 0 : prev + 1))
  }
            ...
            {remainingImagesCount > 0 && (
              ...
              >
                <div className="absolute inset-0 flex flex-col items-center justify-center text-white">
                  ...
                  <span className="text-sm font-medium">{remainingImagesCount} ảnh</span>
                </div>
