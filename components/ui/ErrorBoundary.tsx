...
class ErrorBoundary extends Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  render() {
    ...
    const { children, fallback, apiErrorFallback, networkErrorFallback, authErrorFallback } = this.props
    ...
    if (hasError) {
      ...
      return (
        <div className="flex flex-col items-center justify-center min-h-[300px] p-4 text-center">
          ...
          <p className="text-muted-foreground mb-4">{error?.message || "Đã có lỗi xảy ra. Vui lòng thử lại sau."}</p>
          <button
