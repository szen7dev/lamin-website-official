...
export function OTPVerification({
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>, index: number) => {
    ...
    } else if (newOtp.every((digit) => digit !== "") && newOtp.join("") === "000000") {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-4">
      <div className="w-full max-w-md">
        ...
        <div className="bg-white p-8 rounded-lg shadow-sm">
          <h1 className="text-xl font-semibold text-center mb-2">Nhập mã xác thực</h1>

          <div className="text-center mb-6">
            <p className="text-sm text-gray-600">Mã xác thực được gửi đến số điện thoại</p>
            <p className="font-medium">{formattedPhone} có hiệu lực trong 4 phút 50 giây</p>
          </div>
              ...
              <div key={index} className="w-12 h-12">
                <input
                  ...
                  className={`w-full h-full text-center text-xl font-semibold rounded-full border-2 focus:outline-none ${
                    error ? "border-red-500 text-red-500" : digit ? "border-blue-500 text-blue-500" : "border-gray-300"
                  }`}
                />
              </div>
            ))}
          ...
          <div className="text-center mb-6">
            ...
            <div className="mt-2 text-sm">
              Hoặc có thể{" "}
              <button onClick={onZaloVerification} className="text-blue-600 hover:underline">
                nhận mã qua Zalo
              </button>
            </div>
          </div>
          ...
          <div className="flex justify-center">
            <button onClick={onCancel} className="px-10 py-2 bg-gray-100 text-gray-600 rounded-md hover:bg-gray-200">
              Thoát
            </button>
          </div>
        </div>
